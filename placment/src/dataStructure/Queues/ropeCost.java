...
public class ropeCost {
	public static long minCost(long[] rop, long n) {
		...
		while (Pq.size() > 1) {
			long first = Pq.poll(); /// First minimum element of array (including sum )
			long right = Pq.poll(); /// Second smallest element of array( including sum )
			long sum = first + right;
			ans += sum;
			Pq.add(sum);
		}
		return ans;
